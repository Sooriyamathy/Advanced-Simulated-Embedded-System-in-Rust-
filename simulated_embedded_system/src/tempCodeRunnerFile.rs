...
// Helper function for emergency stop
fn emergency_stop() -> bool {
    let mut input = [0u8; 1];
    io::stdin().read(&mut input).is_ok()
}
...
fn run_simulation(config: &Config) -> std::result::Result<(), Box<dyn Error>> {
    ...
    let mut temperature_values: Vec<f32> = Vec::new(); // Explicit type annotation

    println!("Starting simulation (Press any key to stop)...");
    ...
    loop {
        ...
        if start_time.elapsed() >= Duration::from_secs(30) {
            println!("Simulation completed after 30 seconds.");
            break;
        }
